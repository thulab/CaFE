...
class TextGenerationRunner(BaseHuggingFaceRunner):
    ...
    def forecast(self, sample: SeriesSample, track: TrackKind) -> HuggingFaceForecast:
        ...
        return HuggingFaceForecast(
            ...
            notes={
                "decision": "huggingface_generation",
    ...
    def _build_prompt(self, sample: SeriesSample, track: TrackKind) -> str:
        history = ", ".join(f"{value:.4f}" for value in sample.history[-48:])
        covariate_lines = []
        for name, values in list(sample.covariates.items())[:3]:
            suffix = ", ".join(f"{value:.4f}" for value in values[-48:])
...
class Chronos2Runner(BaseHuggingFaceRunner):
    ...
    def load(self) -> None:
        ...
        try:
            ...
            raise HuggingFaceRunnerError(
                "Chronos-2 support requires transformers, torch, and chronos-forecasting."
            ) from exc
    ...
    def _future_known_covariates(self, sample: SeriesSample) -> set[str]:
        ...
        names: set[str] = set()
        for name in sample.covariates:
            if name == "helpful_covariate":
                names.add(name)
            elif name.startswith("calendar_") or name.startswith("load_"):
                names.add(name)
            elif name.endswith("_known_future"):
                names.add(name)
        return names
    ...
    def _extract_median_prediction(self, prediction: Any, horizon: int) -> list[float]:
        ...
        try:
            if hasattr(prediction, "ndim") and prediction.ndim == 3:
                variate_index = 0
                quantile_index = prediction.shape[1] // 2
                median_values = prediction[variate_index, quantile_index, :]
            elif hasattr(prediction, "ndim") and prediction.ndim == 2:
                quantile_index = prediction.shape[0] // 2
                median_values = prediction[quantile_index, :]
