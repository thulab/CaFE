...
趋势、多季节性、时变季节性、非线性持续性、共同因子、跨序列依赖和协变量冲击响应使用相同的目标距离区间：
...
7. 形成 \(X^{(k)}\) 和 \(Y^{(k)}\)；
8. 分别检查五级与真实 \(X\) 的多上下文距离；intermittency 与 covariate impulse response 还要求五档未来 MASE 标准化 effect RMS 均不低于 0.05；不满足生成资格的整组不生成；
