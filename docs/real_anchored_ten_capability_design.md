# CaFE v15：基于真实基准路径的十项能力公式设计

本文从原始数据开始，完整说明 CaFE v15 如何在 GIFT-Eval 官方测试实例的完整未来标签子集上构造十项能力评测。读者只按本文顺序阅读，就能知道每个量是什么、来自哪里、如何计算，哪些值来自数据采集，哪些值由程序推导，哪些值是人工配置，以及最终怎样得到 treatment 的历史与未来。

本文描述当前代码的实际行为。数据适配实现在 **src/cafe/benchmark_extension/gift_eval.py**，能力公式实现在 **src/cafe/benchmark_extension/mechanisms.py**，紧凑重放契约由 **src/cafe/benchmark_extension/generation.py** 写出。v15 在这里保留的资格规则和五级坐标上增加了共享结构随机性；八项默认能力的结构 family 与冻结字段集中列在 **docs/structural_randomness_v15.md**。
...
CaFE v15 对预测标签采用完整窗口准入：候选实例的整个
...
能力档位只用完整官方历史定义。另根据当前参评模型的实际最大输入长度，检查每个模型真正收到的输入是否既可见又不过强。short 使用下表七个模型；medium 和 long 因 TiRex2 最大输出仅为 320 而将其排除，距离门也相应不再检查 L2048 上下文。模型最大输入长度是协议人工配置，并与推理服务公布的限制一致：
...
现实中可能对应需求规模长期增长、人口或装机容量增长、设备效率缓慢衰减、库存长期消耗等。v15 从全历史线性、延迟线性、缓慢曲率和周期性分段线性中按 seed 选择一种；同一 profile law 覆盖历史与未来，所以预测期允许按历史中的 knot interval、phase 和 slope cycle 自然出现新分段点。

### 3.2 treatment 怎样表示这个概念

对方向稳定的通道 \(d\)，线性 family 定义如下单位斜坡；其他 family 用冻结的延迟、曲率或周期分段 profile 替换时间项，并在历史和未来使用同一个 profile law：
...
令当前 term 的参评模型实际收到的最短上下文为

\[
C_{\min}=\begin{cases}
\min(L,2048),&\text{short},\\
\min(L,8192),&\text{medium/long}.
\end{cases}
\]

其中 short 的 2048 来自 TiRex2，medium/long 排除 TiRex2 后最短上限为
Timer-4.0 和 Chronos-2 的 8192。任一受控周期长度 \(p\) 必须满足
...
3. 检查功率最高的前三个候选，并由 seed 从通过全部门限的候选中选择一个；
...
要求 \(L\ge24\)。对每个通道，用 \([1,q_t]\) 对历史去线性趋势，将残差乘 Hann 窗，计算实数 FFT 功率谱，只保留 \(m\in[2,\lfloor L/4\rfloor]\) 并取功率前 16 个。v15 从前四个 carrier 与其合格调制候选组成的高质量池中抽样。包络候选遍历
...
候选要求增量解释率至少为 0.01、\(\operatorname{Std}(M_d(0{:}L-1))\ge0.01s_d\)，并进入不低于最佳增量解释率一半的 top pool。seed 从幅度调制和一阶周期 phase drift 候选中选择。频率、载波、调制幅度和相位均来自真实历史；未来只延伸同一组解析函数，不读取 \(Y\)。
...
该能力考察模型能否减少对旧制度的依赖、识别最近的新水平并据此预测。它不是短暂异常点；v15 按 seed 选择 step、ramp 或 sigmoid 过渡以及受影响通道，level 仍只控制变点位置。

五级不改变跳变幅度，而是把同一个跳变放到越来越靠近预测起点的位置。等级越高，模型看到的新制度样本越少，任务越难。

### 6.2 treatment 公式与变量

设第 \(k\) 级的跳变位置为 \(j_k\)，方向为 \(v_d\)，共享物理增益为 \(\alpha\)。下式是 step family；ramp 和 sigmoid 用对应的连续过渡 profile 替换指示函数：
...
> 当前保留实现供专项研究使用，但已从默认流水线关闭；默认正式实验不生成、
> 推理或排名该能力。
...
间歇性表示：大部分时间没有额外事件，只有少数时刻出现短脉冲；“可预测”表示脉冲遵循可重复的平均间隔和交替 jitter 规律，模型可以从历史事件间距推断下一次事件。

它考察模型能否从长段普通观测中记住稀疏事件的间隔和相位。现实中可能对应定期补货、批量订单、周期性维护、轮询任务或按固定班次发生的作业。v15 按 seed 选择矩形、三角或指数衰减脉冲、正负方向、通道 mask、宽度和不超过平均间隔 12% 的交替 jitter。

五级控制稀疏度：等级越高，事件间隔通常越长，但五级共享脉冲宽度和幅度。

### 8.2 treatment 公式与变量

设第 \(k\) 级的事件中心集合为 \(\mathcal E_k\)，脉冲宽度为 \(w\)。下式是矩形 family；三角和指数衰减 family 用冻结的宽度内权重替换单位权重：
...
对 \(Z\) 做奇异值分解 \(Z=U\Sigma V^\top\)。先用第一因子解释份额执行面板资格判定，再由 seed 从 PC1 及其与 PC2/PC3 的受限混合和相应稳定载波中选择结构。第一因子解释份额为
...
这里的“驱动”只表示历史预测信息，不能据此声称因果关系。当前实现仍只增强一对通道之间的线性滞后传递，但由 seed 从增量解释率合格的 top pool 中选择，而不是永远使用最强边；它不表示多驱动、非线性传递或反馈网络。
...
当前 treatment 仍表示单向、线性、无反馈的延迟响应，但 v15 会按 seed 选择合法连续协变量、目标、正负映射、事件时刻，以及 exponential、delayed-gamma 或 triangular 响应核和 half-life。

### 12.2 一条样本怎样完成增强和预测

对一个有原生动态协变量的官方实例，主链路如下：

1. 从历史非退化的连续动态协变量和目标通道中按 seed 选择一对通道来承载响应。
...
设历史长度为 \(L\)，预测长度为 \(H\)，选中的协变量通道为 \(j^*\)，目标通道为 \(d^*\)。令 \(p_n\) 表示人为加入的协变量冲击序列。下式是 exponential family 的基准形式；v15 还可选择带 delay 的 gamma 或 triangular 核，三者都冻结为长度 \(2H+1\) 的因果核：
...
这个长度要求保证历史里至少能放下两次冲击，使模型有重复示例可学。对每个协变量通道，先计算其历史去均值 RMS；常数通道或尺度不大于 \(10^{-8}\) 的通道不参与选择，seed 从其余合法连续通道中选择 \(j^*\)，并从原生目标中选择 \(d^*\)。这些选择只决定把同一套受控机制放在哪两个原生通道上，不使用官方未来目标值。
...
- treatment 的目标历史；
- treatment 的真实未来；
- 除被处理 carrier 外的其他协变量路径及其可见性。

消融直接删除被处理的协变量 carrier 整列，而不是构造一条循环移位后的合成路径。这样完整输入与消融输入的唯一差别是该辅助变量是否存在，目标历史中仍保留同一响应，避免把模型对异常 shift 的敏感性误解为协变量利用。

如果完整输入的预测明显优于消融输入，说明模型确实从该协变量获得了增量信息；如果两者几乎没有差别，说明模型可能只依靠目标历史，或者没有学会协变量到目标的延迟映射。common factor 和 cross-series dependence 使用同一原则，但直接只保留被评估的 target 通道，删除其他 target 通道。该归因结果单独报告，不并入 effect NRMSE。
