...
该能力考察模型能否从历史示例中学到“原生协变量冲击 → 目标经过固定滞后核产生响应”，并把同一机制延续到预测区间。增强不再把频率索引合成为日历特征，而是直接使用 GIFT-Eval Arrow 记录已有的 dynamic-real 字段。

历史中放置多次间隔固定的示范冲击，并在预测起点前放置一个末端冲击。它在历史里只暴露响应的前段，剩余响应尾部按构造跨入未来。末端冲击幅度只用历史尺度和 observed mask 求解，使最低强度档的预测期 MASE 标准化 effect RMS 已不低于 0.05，因此机制分母不是靠资格 gate 偶然筛出来。

### 12.2 原生字段与信息边界

适配器保留源 benchmark 的可见性语义：

| Arrow 字段 | CaFE 可见范围 |
|---|---|
| `past_feat_dynamic_real` | 只进入历史请求 |
| `feat_dynamic_real` | 历史与预测期均可见 |

过去不可见的值不会因为增强而变成已知未来。若样本只有 past-only 协变量，模型仍可根据预测起点前冲击预测其响应尾部；只有源记录明确提供 known-future 字段时，预测期协变量路径才会发送给模型。没有原生 dynamic-real 字段的样本直接将本能力记为不可用。

### 12.3 固定因果响应核

设选中的原生协变量通道为 \(j^*\)，目标通道为 \(d^*\)，历史长度为 \(L\)，预测长度为 \(H\)。用脉冲序列 \(p_n\) 表示增强的协变量信号，因果指数核为
...
历史示范脉冲按约 \(2H\) 的间隔向前铺设，最后一个脉冲固定在 \(L-1\)。known-future 样本还可在预测区间继续提供同一种协变量脉冲；past-only 样本的预测期协变量增量恒为零。通道从原始历史选择，尺度也只由历史估计，不读取官方未来目标。

### 12.4 五级 treatment 与输入归因

令 \(P^X,P^Y\) 分别为历史和预测期协变量脉冲分量，\(B^X,B^Y\) 为卷积产生的目标响应。第 \(k\) 级使用同一个物理增益：

\[
Z_X^{(k)}=Z_X+\alpha_kP^X,\qquad
Z_Y^{(k)}=Z_Y+\alpha_kP^Y,
\]
...
五级仍按 2.4 节的完整历史目标距离区间标定。验证会检查五档预测期 MASE 标准化 effect RMS 均不低于 0.05 及所有通用距离约束，但 gate 只验证构造不变量，不负责把不含预测信号的样本“救活”。

输入 ablation 保持 \(X^{(k)}\) 和 \(Y^{(k)}\) 完全一致，也保留原始协变量路径，只对构造出的历史协变量脉冲做确定性时间错位。这样 full 与 ablated 任务的差异直接对应模型是否真正使用了这条协变量冲击证据。
...
原 v9 的日历协变量示例图已退出 v10 协议，不能作为新能力的实验依据。新版图应由包含原生 dynamic-real 字段的新实验生成，并同时展示协变量脉冲、历史示范响应与跨过预测起点的响应尾部。
