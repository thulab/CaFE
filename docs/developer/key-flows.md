...
每个能力维度生成一个 synthetic shard；单变量能力固定 `target_dim=1`，多目标能力使用请求的目标维度（至少 3），`covariate_response` 生成 known-future 协变量列 `weather/event`。每个样本先确定带 `dataset_id` 的 `anchor_profile_id`，再读取该 profile 独立拟合的 nuisance、dataset-local q10/q30/q50/q70/q90 target 和逆标定映射。五档只表示同一 dataset/profile/capability 内的相对强度，不跨 dataset 比较。feature-support 与 near-distance 都只使用所选 profile 自己的真实拆分完成验收；construction、target、feature 和 distance 结果均写入样本 metadata。缺少变量结构、目标档间距或校准条件的 capability 标记为 unsupported，不能在线选择。
...
随后沿用真实数据的 `SeriesPoint` 与 `SampleIndex` 指针存储。读取样本、构造模型输入、forecast、指标、报告和样本曲线都不需要识别合成数据的特殊格式。能力 ID、intensity、seed、预选 `dataset_id/profile_id`、本地 percentile/target、conditioning、latent 参数和 realized features 写入 `SampleIndex.sample_metadata`；shard 级候选 profile 与参数摘要写入 `Shard.generation_config` 与 `runtime/synthetic/*.json`。
