...
def _write_record(out_path, base_url, csv_path, columns, shard, split_config, model_ids, name_by_id, rankings, sample_forecast) -> None:
    ...
    lines.append(f"- **Shard**：target_columns={shard['target_columns']}，sample_count={shard['sample_count']}（序列真值存 SQLite SeriesPoint）")
    lines.append("")
    lines.append("> 通路：CSV/TsFile 输入 → 单目标列摄入 → SQLite SeriesPoint(逐点行) → 指针切片 → ModelInput(无 target_future) → 桩推理 → MSE/MAE/MASE → 榜单。")
