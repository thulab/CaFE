...
import type { ReportDTO, SampleForecastLinkDTO, SampleForecastSort } from '../../api/types';
...
function sampleGroupLabel(link: SampleForecastLinkDTO) {
  const block = (props.report.capability_blocks || []).find((item) => item.capability_block_id === link.capability_block_id);
...
function capabilityBlockLabel(block: NonNullable<ReportDTO['capability_blocks']>[number]) {
