...
describe('resource lifecycle UI', () => {
  ...
  it('renders track-level capability profile and sample forecast links', async () => {
    vi.spyOn(globalThis, 'fetch').mockImplementation(async (input: RequestInfo | URL) => {
      const url = typeof input === 'string' ? input : input instanceof URL ? input.toString() : input.url;
      if (url === '/api/tracks/track-1') {
        return jsonResponse({ track_id: 'track-1', name: 'Synthetic track', primary_metric_id: 'mse', status: 'ready', shard_ids: [], shard_count: 3, sample_count: 30 });
      }
      if (url === '/api/models') return jsonResponse({ items: [] });
      if (url === '/api/benchmarking-runs?limit=200&track_id=track-1') return jsonResponse({ items: [], total: 0, limit: 200, offset: 0 });
      if (url === '/api/tracks/track-1/ranking?metric=mse&policy=latest_valid_result') {
      ...
      if (url === '/api/tracks/track-1/results?sample_link_limit=10&sample_link_offset=0&sample_link_sort=sample_index') {
        return jsonResponse({
          ...
          model_statuses: [],
          model_metrics: [
            { model_id: 'model-a', metrics: { mse: 0.12 } },
            { model_id: 'model-b', metrics: { mse: 0.22 } }
          ],
          capability_blocks: [
            ...
            { capability_block_id: 'block-season', name: 'Seasonality cases', block_type: 'synthetic', capability_type: 'seasonality', capability_label: 'Seasonality', sample_count: 10 },
          ...
          capability_metrics: [
            { task_id: 'task-1', unit_id: 'unit-a', model_id: 'model-a', capability_block_id: 'block-trend', status: 'succeeded', sample_count: 10, metrics: { mse: 0.1 } },
            { task_id: 'task-2', unit_id: 'unit-a', model_id: 'model-a', capability_block_id: 'block-season', status: 'succeeded', sample_count: 10, metrics: { mse: 0.12 } },
            { task_id: 'task-3', unit_id: 'unit-a', model_id: 'model-a', capability_block_id: 'block-noise', status: 'succeeded', sample_count: 10, metrics: { mse: 0.14 } },
            { task_id: 'task-4', unit_id: 'unit-b', model_id: 'model-b', capability_block_id: 'block-trend', status: 'succeeded', sample_count: 10, metrics: { mse: 0.2 } },
            { task_id: 'task-5', unit_id: 'unit-b', model_id: 'model-b', capability_block_id: 'block-season', status: 'succeeded', sample_count: 10, metrics: { mse: 0.22 } },
            { task_id: 'task-6', unit_id: 'unit-b', model_id: 'model-b', capability_block_id: 'block-noise', status: 'succeeded', sample_count: 10, metrics: { mse: 0.24 } }
          ],
          sample_forecast_links: [
            { sample_id: 'sample-1', run_id: 'run-a', sample_index: 0, horizon_start: 12, horizon_end: 16, capability_block_id: 'block-trend', metric_value: 0.12, model_count: 2 }
          ],
