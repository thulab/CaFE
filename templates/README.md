...
第一列 `time` 为时间列（严格递增、等间隔），其余为数值列（目标 + 可选协变量），
全部为有限 float。已用真实 `CsvDatasetReader` 校验通过。

> 加载时在 `target_columns` 里选**恰好一个**目标列；其余数值列作为协变量随全列摄入
> 入库（MVP 不进 sample 视图，但已存在 `SeriesPoint` 中）。
...
| `multivariate_hourly.csv` | 1h | 240 | `time,target,cov_a,cov_b,cov_c` | 多协变量，演示全列摄入 | `target` | 48 / 24 / 24 |
